...
const INITIAL_NOTIFICATIONS: AppNotification[] = [
    ...
    { id: '2', title: 'Radar Active', message: 'Your Professional surveillance is running at full capacity.', type: 'success', isRead: true, time: '1d ago', targetTab: 'profile' },
];
...
const App: React.FC = () => {
  ...
  const handleLogin = (data?: { name: string; email: string; companyName: string }) => {
    if (data) setUser(prev => ({ ...prev, name: data.name, email: data.email, companyName: data.companyName || prev.companyName, isTrial: false, plan: 'Professional' }));
  ...
  const handleConnectQB = () => {
      if (user.isQuickBooksConnected) {
          setUser(prev => ({ ...prev, isQuickBooksConnected: false }));
          handleAddAuditLog('Integration', 'Disconnected from QuickBooks Online', 'warning');
      } else {
          setIsConnectingQB(true);
          setTimeout(() => {
              setIsConnectingQB(false);
              setUser(prev => ({ ...prev, isQuickBooksConnected: true, isXeroConnected: false }));
              handleAddAuditLog('Integration', 'Successfully connected to QuickBooks Online', 'success');
              setNotifications(prev => [{ id: Date.now().toString(), title: 'Sync Successful', message: 'QuickBooks data is now being indexed.', type: 'success', isRead: false, time: 'Just now', targetTab: 'dashboard' }, ...prev]);
          }, 2000);
      }
  };

  const handleConnectXero = () => {
    if (user.isXeroConnected) {
        setUser(prev => ({ ...prev, isXeroConnected: false }));
        handleAddAuditLog('Integration', 'Disconnected from Xero Accounting', 'warning');
    } else {
        setIsConnectingXero(true);
        setTimeout(() => {
            setIsConnectingXero(false);
            setUser(prev => ({ ...prev, isXeroConnected: true, isQuickBooksConnected: false }));
            handleAddAuditLog('Integration', 'Successfully connected to Xero Accounting', 'success');
            setNotifications(prev => [{ id: Date.now().toString(), title: 'Sync Successful', message: 'Xero data is now being indexed.', type: 'success', isRead: false, time: 'Just now', targetTab: 'dashboard' }, ...prev]);
        }, 2000);
    }
  };
  ...
  const handleExport = (format: 'csv' | 'pdf') => {
      alert(`Generating Full Forensic Audit Trail in ${format.toUpperCase()} format...`);
  ...
  const handlePaymentSuccess = () => {
      if (selectedPlan) {
          ...
          handleAddAuditLog('Upgrade', `Plan confirmed: ${selectedPlan.name}`, 'success');
  ...
  const renderCurrentView = () => {
      ...
      return (
        ...
        >
            {activeTab === 'dashboard' && <Dashboard scanHistory={scanHistory} user={user} onConnectQuickBooks={handleConnectQB} setActiveTab={setActiveTab} />}
            {activeTab === 'scan' && <ScanManager user={user} onAddAuditLog={handleAddAuditLog} onScanComplete={handleScanComplete} onUpgradePrompt={handleUpgradeClick} onUpdateUser={setUser} />}
            ...
            {activeTab === 'help' && <div className="h-[calc(100vh-12rem)] bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden"><HelpCenter isOpen={true} onClose={() => setActiveTab('dashboard')} /></div>}
            {activeTab === 'audit' && (
                <div className="space-y-6">
                    <div className="flex justify-between items-center">
                        <div>
                            <h2 className="text-2xl font-bold text-slate-800">Audit Trail</h2>
                            <p className="text-slate-500">Forensic-grade action tracking (Professional Enabled).</p>
                        </div>
